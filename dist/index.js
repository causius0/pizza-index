...
const members = JSON.parse(readFileSync(new URL('../data/avpn-members.json', import.meta.url), 'utf-8'));
const pricedById = new Map(PRICED.map((p) => [p.id, p]));
const obsByPizzeria = new Map();
for (const o of OBSERVATIONS)
    obsByPizzeria.set(o.pizzeriaId, o);
...
for (const m of members) {
    ...
    e.n += 1;
    if (e.names.length < 60)
        e.names.push(`${m.name} (${m.city}) [#${m.number}]`);
    census.set(iso, e);
}
const byCountry = new Map();
const currByCountry = new Map();
for (const o of OBSERVATIONS) {
    if (o.includeInAverage === false)
        continue;
    const p = pricedById.get(o.pizzeriaId);
    if (!p || p.certification.source !== 'AVPN')
        continue;
...
const fmt = (v, d = 2) => v === null ? '—' : v.toLocaleString('en-US', { maximumFractionDigits: d, minimumFractionDigits: 0 });
...
const listHtml = [...census.entries()].map(([c, e]) => `<h3>${c} — ${e.n} AVPN-certified</h3><p><small>${e.names.join(' · ')}</small></p>`).join('\n');
